 ...
 * @return string html
 */
function tvc_show_wp_error( $message, $dismissible = false, $permanent_dismissible_id = '' ) {
	return tvc_show_wp_message( $message, 'error', $dismissible, $permanent_dismissible_id );
}
 ...
 * @return string html
 */
function tvc_show_wp_warning( $message, $dismissible = false, $permanent_dismissible_id = '' ) {
	return tvc_show_wp_message( $message, 'warning', $dismissible, $permanent_dismissible_id );
}
 ...
 * @return string html
 */
function tvc_show_wp_success( $message, $dismissible = false, $permanent_dismissible_id = '' ) {
	return tvc_show_wp_message( $message, 'success', $dismissible, $permanent_dismissible_id );
}
 ...
 * @return string html
 */
function tvc_show_wp_info( $message, $dismissible = false, $permanent_dismissible_id = '' ) {
	return tvc_show_wp_message( $message, 'info', $dismissible, $permanent_dismissible_id );
}
 ...
 * @return string html
 */
function tvc_show_wp_message( $message, $type, $dismissible, $permanent_dismissible_id ) {
 ...
 * @return string html
 */
function tvc_handle_wp_errors_response( $response, $message ) {
	...
	tvc_write_log_file( $message . ' ' . $error_text );

	return tvc_show_wp_error( $message . ' Error message: ' . $error_message );
}
 ...
 */
function tvc_write_log_file( $error_message, $filename = 'error' ) {
	$file = ENHANCAD_PLUGIN_DIR . $filename . '.log';
	...
	if ( false === file_put_contents( $file, date( 'Y-m-d H:i:s', current_time( 'timestamp' ) ) . ' - ' . ucfirst( $filename ) . ' Message: ' . $message_line . PHP_EOL, FILE_APPEND ) ) {
		...
		tvc_show_wp_error( sprintf( __( 'There was an error but I was unable to store the error message in the log file. The message was %s', 'tvc-product-feed-manager' ), $error_message ) );
	}
}
 ...
 * @return string html
 */
function tvc_update_your_woocommerce_version_message() {
 ...
 * @return string html
 */
function tvc_you_have_no_woocommerce_installed_message() {
	$message_code  = '<div class="full-screen-message-field">';
	...
	/* translators: %s: link to information about the WooCommerce plugin */
		$message_code .= '<p>' . sprintf( __( 'You can find more information about the WooCommerce Plugin %sby clicking here</a>.', 'tvc-product-feed-manager' ), '<a href="https://wordpress.org/plugins/woocommerce/">' ) . '</p>';
	$message_code .= '</div>';
 ...
 * @return string
 */
function tvc_log_http_requests( $response, $args, $url ) {
	if ( false !== is_wp_error( $response ) && tvc_on_any_own_plugin_page() ) {
		$logfile = ENHANCAD_PLUGIN_DIR . 'http_request_error.log';
...
add_filter( 'http_response', 'tvc_log_http_requests', 10, 3 );
