...
/**
 * TVC Product Feed Form Control Class.
 *
...
if ( ! class_exists( 'TVC_Feed_Form_Control' ) ) :

	/**
	 * TVC Feed Form Control Class
	 */
	class TVC_Feed_Form_Control {
		...
		public static function source_selector() {
			...
			if ( ! empty( $sources ) ) {
				...
				$html_code .= '<option value="0">' . esc_html__( 'Select your product source', 'tvc-product-feed-manager' ) . '</option>';
		...
		public static function channel_selector() {
			...
			if ( ! empty( $channels ) ) {
				...
				$html_code .= '<select class="tvc-main-input-selector" id="tvc-merchants-selector" style="display:initial;">';

				$html_code .= '<option value="0">' . esc_html__( '-- Select your merchant --', 'tvc-product-feed-manager' ) . '</option>';
				...
				$html_code = esc_html__( 'You first need to install a channel before you can add a feed. Open the Manage Channels page and install at least one channel.', 'tvc-product-feed-manager' );
			}
		...
		public static function country_selector() {
			...
			if ( ! empty( $countries ) ) {
				$html_code  = '<select class="tvc-main-input-selector tvc-countries-selector" id="tvc-countries-selector" disabled>';
				$html_code .= '<option value="0">' . esc_html__( '-- Select your target country --', 'tvc-product-feed-manager' ) . '</option>';
		...
		public static function schedule_selector() {
			$html_code  = '<span id="tvc-update-day-wrapper" style="display:initial">' . esc_html__( 'Every', 'tvc-product-feed-manager' ) . ' ';
			$html_code .= '<input type="text" class="small-text" name="days-interval" id="days-interval" value="1" style="width:30px;" /> ' . esc_html__( 'day(s) at', 'tvc-product-feed-manager' ) . '</span>';
			$html_code .= '<span id="tvc-update-every-day-wrapper" style="display:none">' . esc_html__( 'Every day at', 'tvc-product-feed-manager' ) . '</span>';
			...
			$html_code .= '<span id="tvc-update-frequency-wrapper" style="display:initial">';
			$html_code .= ' ' . esc_html__( 'for', 'tvc-product-feed-manager' ) . ' ';
			$html_code .= '<select id="update-schedule-frequency" style="width:50px;height:35px;">' . self::frequency_list() . '</select>';
			$html_code .= ' ' . esc_html__( 'time(s) a day', 'tvc-product-feed-manager' );
			$html_code .= '</span>';
	...
	// end of TVC_Feed_Form_Control class
